"""LCM type definitions
This file automatically generated by lcm.
DO NOT MODIFY BY HAND!!!!
"""

try:
    import cStringIO.StringIO as BytesIO
except ImportError:
    from io import BytesIO
import struct

class message_t(object):
    __slots__ = ["timestamp", "danger"]

    __typenames__ = ["int64_t", "boolean"]

    __dimensions__ = [None, None]

    def __init__(self):
        self.timestamp = 0
        self.danger = False

    def encode(self):
        buf = BytesIO()
        buf.write(message_t._get_packed_fingerprint())
        self._encode_one(buf)
        return buf.getvalue()

    def _encode_one(self, buf):
        buf.write(struct.pack(">qb", self.timestamp, self.danger))

    def decode(data):
        if hasattr(data, 'read'):
            buf = data
        else:
            buf = BytesIO(data)
        if buf.read(8) != message_t._get_packed_fingerprint():
            raise ValueError("Decode error")
        return message_t._decode_one(buf)
    decode = staticmethod(decode)

    def _decode_one(buf):
        self = message_t()
        self.timestamp = struct.unpack(">q", buf.read(8))[0]
        self.danger = bool(struct.unpack('b', buf.read(1))[0])
        return self
    _decode_one = staticmethod(_decode_one)

    _hash = None
    def _get_hash_recursive(parents):
        if message_t in parents: return 0
        tmphash = (0x5f2fbce8d4e749e2) & 0xffffffffffffffff
        tmphash  = (((tmphash<<1)&0xffffffffffffffff) + (tmphash>>63)) & 0xffffffffffffffff
        return tmphash
    _get_hash_recursive = staticmethod(_get_hash_recursive)
    _packed_fingerprint = None

    def _get_packed_fingerprint():
        if message_t._packed_fingerprint is None:
            message_t._packed_fingerprint = struct.pack(">Q", message_t._get_hash_recursive([]))
        return message_t._packed_fingerprint
    _get_packed_fingerprint = staticmethod(_get_packed_fingerprint)

